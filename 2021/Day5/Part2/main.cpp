...
int main(){
    ...
    file.close();
    int map[990][990];
    memset(map, 0, sizeof(int) * 990 * 990);
    int *max = (int*) malloc(sizeof(int) * 2), *min = (int*) malloc(sizeof(int) * 2);
    count = 0;
    ...
    for(int i = 0; i < input.size(); i += 2){
        ...
        if(max[0] != min[0] && max[1] == min[1]){
            ...
            for(int inc = 0; inc <= max[0] - min[0]; inc++){
                if(++map[input[i][0] + (input[i][0] == min[0] ? inc : -inc)][input[i][1] + (input[i][1] == min[1] ? inc : -inc)] == 2)
                    count++;
            }
        }
        free(input[i]);
