...
int main() {
    ...
    struct Vector * seeds = init_vector();
        ...
        switch (mode) {
            case SEED:
                for (size_t i = 7; i < read;) { // 7 is the length of "seeds: "
                    sscanf(line + i, "%ld%n", &dest_start, &src_start);
                    vector_push(seeds, dest_start); // dest_start is the read seed in this case
                    i += src_start + 1; // src_start is the local offset to the next number
                }
                break;
            case SEED_TO_SOIL:
    ...
    for (size_t i = 0; i < seeds->size; ++i) {
        size_t location = convert_seed_to_location(vector_at(seeds, i));
        if (location < min) 
            min = location;
    }
