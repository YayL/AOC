...
unsigned long long calculate_fence_cost(size_t x_start, size_t y_start) {
    ...
    // scan perimiter horizontally
    for (size_t y = 0; y < HEIGHT; ++y) {
        for (size_t x = 0; x < WIDTH; ++x) {
            if (region[y][x] == 1) {
                perimiter += x == 0 || region[y][x - 1] == 0;
                perimiter += x + 1 == WIDTH || region[y][x + 1] == 0;
            }
        }
    }

    // scan perimiter vertically
    for (size_t x = 0; x < WIDTH; ++x) {
        for (size_t y = 0; y < HEIGHT; ++y) {
            if (region[y][x] == 1) {
                perimiter += y == 0 || region[y - 1][x] == 0;
                perimiter += y + 1 == HEIGHT || region[y + 1][x] == 0;
            }
        }
    }
